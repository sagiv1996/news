  ...
  <v-list>
    <v-list-item>
      <v-text-field
        ...
        clearable
        v-model.lazy="string"
        @click:clear="string = null"
      ></v-text-field>
    </v-list-item>
    <v-virtual-scroll
      :items="filteredItems"
      :item-height="50"
      :max-height="$vuetify.breakpoint.mobile? '100vh' : '75vh'"
    >
      <template v-slot="{ item }">
        <v-list-item dense :to="switchLocalePath(item.code)" nuxt link>
          ...
          <v-spacer></v-spacer>
          <v-list-item-subtitle>{{item.name}} ({{item.iso}})</v-list-item-subtitle>
        </v-list-item>
        
      </template>
    </v-virtual-scroll>
    <v-list-item v-if="filteredItems.length === 0">
          <v-list-item-title>No results were found for the values ​​you searched for
          </v-list-item-title>
        </v-list-item>
  </v-list>
</template>
